...
pub fn process(platform: &constant_mod_priv::Platform, c1c2ad: &Vec<(Vec<u8>, Vec<u8>)>) {
    let (c1, c2) = &c1c2ad[0];
    constant_mod_priv::Platform::process(&platform.k_p, &platform.sk_p, &c1, &c2, &None, &CTX.to_vec());
}
