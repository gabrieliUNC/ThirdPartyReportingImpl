...
pub fn read(clients: &Vec<basic::Client>, c1c2ad: &Vec<(Vec<u8>, Vec<u8>, u32)>, pks: &Vec<PKey<Public>>, sigma_st: &Vec<(Vec<u8>, (Vec<u8>, u32))>) {
...
pub fn bench_basic_read(c: &mut Criterion) {
    ...
    for (i, msg_size) in LOG_SCALE.iter().enumerate() {
        group.bench_with_input(format!("Read message of size {}", msg_size), msg_size, |b, &msg_size| {
